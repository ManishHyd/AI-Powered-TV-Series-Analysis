...
class BlogSpider(scrapy.Spider):
    ...
    def parse(self, response):#Iterates ove the links in the web page
        ...
        for next_page in response.css('a.mw-nextlink'): #Iterates over pages
            yield response.follow(next_page, self.parse)

    def parse_jutsu(self, response):
        jutsu_name = response.css("span.mw-page-title-main::text").extract()[0] #Extract the first 
        jutsu_name = jutsu_name.strip() #strips the spaces before and afterwards
